...
class ABIFReader:
    def __init__(self, fn):
        ...
        if self.type != 'ABIF':
            self.close()
            raise FileFormatError("error: No ABIF file '%s'" % fn)
    ...
    def getData(self, name, num=1):
        ...
        if not entry:
            raise EntryNotFound("error: Entry '%s (%i)' not found in '%s'" % (name, num, self.filename))
    ...
    def readNextpString(self):
        ...
        chars = [char.decode("utf-8") for char in chars]
        return ''.join(chars)
